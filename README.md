...
-Submit -> 0.495（なんで...? 学習時のエラーも解消されたけど。**全部の特徴量を用いた方がスコアが高いということは、何か大事な特徴量を欠落させてしまっている可能性**）
  - 特徴量数減少
  - 目的変数の不均衡対策
  - 特徴量の重要度表
...
-TODO
  - **学習時の特徴量寄与** -> OK
  - **TF-IDFの上位の単語を確認** -> OK
  - **業界は名義変数だが、One-hotにしている現在は29次元になっているため、ラベルエンコーディングに変更する** -> OK
  - 
### 20251020
- 業界列を購入確率降順のラベルエンコーディング化
- TF-IDFから有用な特徴量の探索
- Train.csvを見ると、「今後のDX展望」は購入フラグと関係ないかもしれん

### 20251021
【モデル】
ランダムフォレスト＋LightGBM + () →多数決
・ランダムフォレスト：母集団から重複を許してランダムにサンプリングする（学習データ群1,2,3,..）。各学習データ群に対してそれぞれ異なる一部の特徴量のみを用いて決定木をつくる（相関が弱く少しずつ違った決定木ができる）。各決定木の多数決をとって最終的な出力にする、

【企業内容】
頻出無意味単語：当社、会社概要・企業、業界

仮説：以下の単語が購入フラグに寄与しているのでは？それぞれを特徴量とするのは多いから、ポジティブ用語群：ネガティブ傾向用語群の割合とか？
未購入:慎重、必要性、見極め、 
購入：機動的、スピード、迅速、柔軟、加速
...
【アンケート】
前処理：アンケート７はアンケート６がはいの場合のみ値がある
仮説：アンケート6が１かつアンケート7が１～３
